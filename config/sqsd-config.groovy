/**
 * Custom config for sqsd.
 * To specify custom property values,
 * substitute property value comments with the real values and uncomment.
 **/

//aws.access_key_id = the aws access key id
//aws.secret_access_key = the aws secret access key
//sqsd.queue.name = name of the aws sqs queue
//sqsd.queue.url = url of the aws sqs queue (Required)
//sqsd.queue.region_name = region name of the aws sqs queue
//sqsd.max_messages_per_request = messages to retrieve per request (max: 10)
//sqsd.wait_time_seconds = long polling wait time when querying the queue (max: 20)
//sqsd.http.host = the host url where to POST the retrieved messages (Required)
//sqsd.http.path = the path of the host's enpoint where to POST the retrieved messages (Required)
//sqsd.http.request.content_type = the type of the request's content

